...
async def count_breezy(page):
    return await page.evaluate(r'''() => {
        const cards = document.querySelectorAll('li.job, [data-breezy-job], .breezy-job-card');
        if (cards.length > 0) return cards.length;
        const links = Array.from(document.querySelectorAll('a[href*="/p/"]'));
        const unique = new Set(links.map(a => a.href.split('?')[0].split('#')[0].replace(/\/$/, '').toLowerCase()));
        return unique.size > 0 ? unique.size : document.querySelectorAll('.position').length;
    }''')
...
async def count_recruitee(page):
    ...
    return await page.evaluate(r'''() => {
        ...
        const uniqueJobs = new Set(
            links
                .map(a => a.href.split('?')[0].split('#')[0].replace(/\/$/, '').toLowerCase())
                .filter(href => href.includes('/o/'))
        );
        if (uniqueJobs.size > 0) return uniqueJobs.size;
        return document.querySelectorAll('.opening').length;
    }''')
...
async def count_workable(page):
    ...
    return await page.evaluate(r'''() => {
        ...
        const unique = new Set(links.map(a => a.href.split('?')[0].replace(/\/$/, '').toLowerCase()));
        if (unique.size > 0) return unique.size;
        return document.querySelectorAll('[data-test="job-item"]').length;
    }''')
...
async def get_count(page, url):
    ...
    # Fast-track Kalypso singular job descriptions directly to 1
    if "kalypsomedia.com" in u and "id=" in u:
        return 1
    ...
    elif "recruitee.com" in u:     count = await count_recruitee(page)
    ...
    # Custom parser for Kalypso Media main recruitment tracking dashboard
    elif "kalypsomedia.com" in u:
        count = await page.evaluate(r'''() => {
            const links = Array.from(document.querySelectorAll("a")).map(a => a.href);
            const uniqueJobs = new Set(links.filter(href => href.includes('?id=') || href.includes('&id=')));
